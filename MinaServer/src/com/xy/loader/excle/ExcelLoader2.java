 ...
 * 
 */
public class ExcelLoader2 implements Loader
{
	private Logger log = Logger.getLogger(ExcelLoader2.class);
	...
	public ExcelLoader2(LoadHandler2 handler, File file)
	{
	...
	public void load(LoadErrorHandler err) throws Exception
	{
		...
		try
		{
			...
			// 再读入每行
			boolean first = true; // 将发现的第一个异常输出到日志中
...
//		else
	...
	private String getCellString(HSSFCell cell)
	{
		...
		if (type == HSSFCell.CELL_TYPE_FORMULA)	
			type = cell.getCachedFormulaResultType(); //若是公式格式，则读取公式结果的存放格式
