 ...
 * 
 * <p>
 * 第一行是大标题行，第二行是列标题行，从第三行开始是数据行，直到遇到“###记录完毕###”。 后面的是注释信息
 * 
 * <p>
 ...
 * 
 */
public class ExcelLoader3 implements Loader
{
	private Logger log = Logger.getLogger(ExcelLoader3.class);

	private static final String DATA_END = "###记录完毕###";

	private static final DateFormat df = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");

	private LoadHandler2 handler;
	private File file;
	...
	public ExcelLoader3(LoadHandler2 handler, File file)
	{
	...
	public void load(LoadErrorHandler err) throws Exception
	{
		...
		try
		{
			...
			if (rows <= 2) throw new Exception("文件没有内容");
			...
			for (int i = m; i < n; i++)
			{
				...
				titles[i - m] = cell.getRichStringCellValue().getString();
			}

			handler.onTitle(titles);
